 ...
 */
export default function Members({ ds, bench, month }: { ds: Dataset; bench: Bench; month: string }) {
  const [mode, setMode] = useState<'visit' | 'tx'>('visit')
  const [overcount, setOvercount] = useState(0)   // assumed over-count in anonymous visits, %

  const strata = useMemo(() => ds.venues.map(v => {
    const m = bench.get(v, ALL, month, 'member')
    const n = bench.get(v, ALL, month, 'nonmember')
    return {
      key: v,
      aNum: m.rev, aDen: mode === 'visit' ? m.vis : m.tx,
      bNum: n.rev, bDen: (mode === 'visit' ? n.vis : n.tx) * (1 - overcount / 100),
    }
  }).filter(s => s.aDen > 0 && s.bDen > 0), [ds, bench, month, mode, overcount])
  ...
  return (
    <>
      <Standfirst
        ...
        sub="The gap is real and it survives everything below. The size of it does not, and the difference between those two statements is the difference between a defensible number and a slide."
      />
      ...
      <Finding tone={positive === nG ? 'up' : 'flat'}>
        Members spend more at <b>{positive} of {nG}</b> venues
        {positive === nG && <> — every single one</>}.
        {' '}That consistency is the strong claim: under a null of no systematic difference the chance of all {nG} venues
        ...
        {' '}The <i>size</i> is the weak claim: the per-venue gap runs from ${Math.min(...r.gaps.map(g => g.gap)).toFixed(2)} to
        {' '}${Math.max(...r.gaps.map(g => g.gap)).toFixed(2)}, a spread of ${spread.toFixed(2)}, and the headline moves
        ${choiceSpread.toFixed(2)} depending only on how the venues are combined.
      </Finding>
      ...
      <StatGrid>
        ...
        <Stat label="Within-venue gap" value={'$' + r.weighted.toFixed(2)} foot="the same venues, compared inside themselves first" />
        <Stat label="Venues where it reverses" value={n0(r.reversals)} tone={r.reversals ? 'bad' : ''}
          foot={r.reversals ? 'the pooled figure has the sign wrong somewhere' : 'the direction is the same everywhere, so pooling does not flip it here'} />
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <div className="card-t">Every venue, on its own</div>
        <div className="card-s">
          The member minus non-member gap at each venue. This is the picture the pooled number replaces with one figure.
        </div>
        ...
        <div className="chart-note">
          ...
          eleven venues, not a confidence interval on any one of them.
        </div>
      </div>
      ...
      <Ledger
        ...
        is={<>
          Three things have to travel with that number or it should not travel at all: it is pooled, and pooling is a
          weighting choice worth ${choiceSpread.toFixed(2)} here; the non-member denominator over-counts, so the gap is an
          upper bound; and members chose to become members, so this is a description of two groups, not a measurement of
...
sign test    P(all ${nG} venues agree | no systematic difference) = 2 · (1/2)^${nG}`}>
        <p>
          ...
          {' '}{pct(Math.min(...ds.venues.map(v => { const m = bench.get(v, ALL, month, 'member'); const n = bench.get(v, ALL, month, 'nonmember'); return m.vis + n.vis ? m.vis / (m.vis + n.vis) : 1 })), 0)} to
          {' '}{pct(Math.max(...ds.venues.map(v => { const m = bench.get(v, ALL, month, 'member'); const n = bench.get(v, ALL, month, 'nonmember'); return m.vis + n.vis ? m.vis / (m.vis + n.vis) : 0 })), 0)} of visits.
